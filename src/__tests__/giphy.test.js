...
it('should fetch gifs given a query', () => {
  fetch(
    `http://api.giphy.com/v1/gifs/search?api_key=${process.env.GIPHY_KEY}&q=dog&limit=5`
  )
    .then((res) => res.json())
    .then((json) => {
...
it('fetches a gif given the id', () => {
  fetch(
    `http://api.giphy.com/v1/gifs/xTiQyqwwF7R3bwm2Ag?api_key=${process.env.GIPHY_KEY}`
  )
    .then((res) => res.json())
    .then((json) => {
